...
from app.models.platform import CrawlerProfile, JobCrawlRecord, JobCrawlSession
...
async def run_kaggle_import(db: AsyncSession, session_id: int, dataset_ref: str) -> dict[str, Any]:
    """Download a Kaggle dataset, store records, run job analytics."""
    session = await db.get(JobCrawlSession, session_id)
    if session is None:
        raise ValueError(f"session {session_id} not found")
    ...
    try:
        ...
        session.error = str(exc)
        await db.commit()
        log.error("jobs.kaggle.failed", session_id=session_id, error=str(exc))
        raise

...
function ProjectButton({
  ...
  const cls =
    size === "md"
      ? "inline-flex items-center gap-1.5 rounded-theme bg-primary text-white text-sm font-medium px-4 py-2 hover:opacity-90 transition-opacity"
      : "inline-flex items-center gap-1 rounded-theme bg-primary text-white text-xs font-medium px-3 py-1.5 hover:opacity-90 transition-opacity";

  if (launchUrl) {
    return (
      <a href={launchUrl} rel="noopener noreferrer" className={cls} onClick={(e) => e.stopPropagation()}>
        View project ↗
      </a>
    );
  }
  ...
  return (
    <button
      ...
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        toast.info("Project coming soon", "This project is still in development. Check back later!");
      }}
    >
      View project
    </button>
...
function PostCard({ p, serviceUrlMap }: { p: BlogPost; serviceUrlMap: Record<string, string> }) {
  const launchUrl = p.service_key ? (serviceUrlMap[p.service_key] ?? null) : null;
  return (
    <div className="group rounded-2xl bg-surface border border-white/10 shadow-card overflow-hidden hover:border-primary/50 transition-colors flex flex-col">
      ...
      <div className="px-5 pb-4">
        <ProjectButton launchUrl={launchUrl} />
      </div>
    </div>
  );
}
    ...
    <div className="group w-full rounded-2xl bg-surface border border-primary/25 shadow-card overflow-hidden hover:border-primary/60 transition-all hover:shadow-[0_8px_40px_rgba(var(--color-primary-rgb,204,2,2),0.15)]">
      <Link href={`/blog/${p.slug}`} className="grid md:grid-cols-5">
        ...
        <div className="md:col-span-3 p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
          ...
          {p.category && <span className="text-xs text-secondary mb-2">{p.category.name}</span>}
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-muted mt-auto pt-4 border-t border-white/5">
            ...
            <span>♥ {p.like_count}</span>
            <span>💬 {p.comment_count}</span>
            <span className="ml-auto text-primary text-sm group-hover:translate-x-1 transition-transform">
              Read →
            </span>
          </div>
          <div className="mt-4">
            <ProjectButton launchUrl={launchUrl} size="md" />
          </div>
        </div>
      </Link>
    </div>
...
export default function BlogListPage() {
  ...
  const serviceUrlMap = useMemo(() => {
    ...
    (boot?.microservices ?? []).forEach((s) => {
      if (s.key && s.base_url) m[s.key] = s.base_url;
    });
    return m;
  }, [boot?.microservices]);
